...
def build_track_info(track_availability):
    ...
    track_info.sort(
        key=lambda t: (0 if t["impossible"] else (1 if t["requires_roll"] else 2))
    )
    return track_info
...
def clear_pending_session(request):
    ...
    keys = [
        ...
        "pending_int_mod",
        "pending_wis_mod",
        "pending_social_class",
        "pending_sub_class",
        "pending_wealth_level",
        "pending_attribute_focus",
        "pending_return_to_generator",
    ]
    for key in keys:
...
def store_character_in_session(request, character):
    ...
    for yr in character.prior_experience.yearly_results:
        skills.append(yr.skill_gained)
    request.session["interactive_skills"] = skills
    request.session["interactive_skill_points"] = (
        character.prior_experience.total_skill_points
    )
    ...
    request.session["interactive_initial_skills"] = list(
        character.skill_track.initial_skills
    )
