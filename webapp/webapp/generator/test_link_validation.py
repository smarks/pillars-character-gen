...
class MarkdownLinkValidationTests(TestCase):
        ...
        for link_text, link_url in links:
            ...
            if link_url.startswith('/'):
                ...
                if link_url in valid_links:
                    ...
                    try:
                        reverse(url_name, kwargs={'name': link_url.split('/')[-2]} if 'html/' in link_url else {})
                    except (NoReverseMatch, KeyError):
                        # Try direct URL resolution
                        try:
                            resolve(link_url)
                        except Resolver404:
                            self.fail(f"Invalid link in welcome.md: [{link_text}]({link_url})")
                elif link_url == '/':
                    # Root URL should resolve to welcome
                    try:
                        resolve(link_url)
                    except Resolver404:
                        self.fail(f"Root URL / should be valid, but link [{link_text}]({link_url}) failed")
                else:
    ...
    def test_about_md_links_are_valid(self):
        ...
        for link_text, link_url in links:
            # Only check internal links (starting with /)
            if link_url.startswith('/') and not link_url.startswith(('http://', 'https://')):
                # Check for common broken patterns
                if link_url == '/':
                    broken_links.append(f"[{link_text}]({link_url}) - should be /generator/")
                else:
                    # Try to resolve the URL
                    try:
                        resolve(link_url)
                        ...
                        if link_url.startswith('/html/'):
                            name = link_url.split('/')[-2] if link_url.endswith('/') else link_url.split('/')[-1]
                            try:
                                reverse('reference_html', kwargs={'name': name})
                            except NoReverseMatch:
                                broken_links.append(f"[{link_text}]({link_url}) - invalid reference_html name: {name}")
                        else:
                            broken_links.append(f"[{link_text}]({link_url}) - URL not found")
                    ...
                    try:
                        ...
                        if link_url.startswith('/html/'):
                            ...
                            try:
                                ...
                                broken_links.append(f"[{link_text}]({link_url}) - turn_sequence URL not found")
                        else:
                            broken_links.append(f"[{link_text}]({link_url}) - URL not found")
        
        if broken_links:
            self.fail(f"Found broken links in public-rulebook.md:\n" + "\n".join(broken_links))
    ...
    def test_no_root_links_to_generator(self):
        ...
        for filename in markdown_files:
            ...
            for link_text, link_url in matches:
                broken_links.append(f"{filename}: [{link_text}]({link_url}) should be /generator/")
        ...
        for filename in markdown_files:
            ...
            for link_text, link_url in links:
                ...
                try:
                    ...
                    broken_links.append(f"{filename}: [{link_text}]({link_url}) - name '{name}' not found")
                
                # Check if the markdown file exists
                md_file = os.path.join(self.references_dir, f"{name}.md")
                if not os.path.exists(md_file):
                    broken_links.append(f"{filename}: [{link_text}]({link_url}) - file {name}.md does not exist")
...
class TemplateLinkValidationTests(TestCase):
    ...
    def test_all_url_names_exist(self):
        ...
        if missing_urls:
            self.fail(f"URL names used in templates but not defined: {', '.join(missing_urls)}")
...
class URLResolutionTests(TestCase):
    ...
    def test_reference_html_urls_work(self):
        ...
        for name in reference_names:
            try:
                ...
                md_file = os.path.join(settings.BASE_DIR, '..', 'references', f"{name}.md")
                if not os.path.exists(md_file):
                    broken_urls.append(f"reference_html('{name}') - file {name}.md does not exist")
            except NoReverseMatch as e:
    ...
    def test_all_template_url_tags_are_valid(self):
        ...
        if broken_urls:
            self.fail("Found invalid {% url %} tags in templates:\n" + "\n".join(broken_urls))
    ...
    def test_dm_handbook_links_are_valid(self):
        ...
        if broken_links:
            self.fail(f"Found broken links in dm-handbook.md:\n" + "\n".join(broken_links))
