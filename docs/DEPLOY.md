...
If you're seeing "CSRF verification failed" errors:

**1. Check your `.env` settings:**

```bash
# If NOT using HTTPS (testing)
...
**Common CSRF causes:**
- `CSRF_TRUSTED_ORIGINS` not set or wrong protocol (http vs https)
- `DJANGO_USE_HTTPS=True` but server is HTTP (cookies won't be set)
- `ALLOWED_HOSTS` doesn't include your server's IP/domain
